...
class TWMarkerSurfaceView extends TWSurfaceViewBase {
    ...
    private Bitmap processFrameForMarkers(VideoCapture capture, DtouchMarker marker) {
      	switch (TWCameraActivity.viewMode) {
           case TWCameraActivity.VIEW_MODE_MARKER:
	        	processFrameForMarkersFull(capture, marker);
	            break;
    ...
    private void displayMarkersDebug(Mat imgMat, Scalar contourColor, Scalar codesColor){
    	...
    	if (markerFound){    	
    		...
    		Imgproc.drawContours(mRgba, components, marker.getComponentIndex(), contourColor, 3, 8, hierarchy, 2, new Point(0,0));
    	}
       	if (components != null)
        	components.clear();
        if (hierarchy != null)
		...
		try
        {
        	...
        	while (Thread.currentThread() == mThread) {
        		...
        		synchronized (this) {
        			...
        			if(!mMarkerDetected){
            			bmp = processFrameForMarkers(mCamera, dtouchMarker);
        			}else{
